...
def create_access_token(user_id:str,password:str,roles:str, secret_key: str, algorithm: str="HS256",expire_minutes:int=15) -> str:
    """Create a JWT access token."""
    logger.info(f"Creating access token for user_id: {user_id}")
    data = {"user_id": user_id, "password": password,"roles":roles,"expires_in": time.time() + expire_minutes * 60}
